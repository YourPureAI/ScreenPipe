...
impl AudioManager {
    ...
    async fn start_internal(&self) -> Result<()> {
        *self.status.write().await = AudioManagerStatus::Running;
        ...
        let capture_only = self.options.read().await.capture_only;

        if !capture_only {
            // In capture-only mode we never start the transcription receiver —
            // the model is never loaded and no Whisper/Deepgram calls are made.
            let mut transcription_receiver_handle =
                self.transcription_receiver_handle.write().await;
            *transcription_receiver_handle =
                Some(self.start_transcription_receiver_handler().await?);
        }
        ...
        *recording_receiver_handle = Some(self.start_audio_receiver_handler().await?);
        ...
        // Reconciliation sweep is only meaningful in full / process-only mode
        if !capture_only && self.options.read().await.transcription_mode == TranscriptionMode::Batch {
    ...
    async fn start_audio_receiver_handler(&self) -> Result<JoinHandle<()>> {
        ...
        let capture_only = options.capture_only;
        let queue_writer = options.queue_writer.clone();
        drop(options);
        ...
        // When running as a thin client we skip model loading entirely and
        // write raw PCM packets straight to the queue directory.
        if capture_only {
            let qw = queue_writer.ok_or_else(|| {
                anyhow::anyhow!("capture_only=true but no queue_writer configured")
            })?;
            return Ok(tokio::spawn(async move {
                while let Ok(audio) = whisper_receiver.recv() {
                    metrics.record_chunk_received();

                    // Audio-based call detection kept active even in capture-only
                    ...
                    let packet = screenpipe_queue::RawAudioPacket {
                        pcm_samples: audio.data.as_ref().to_vec(),
                        sample_rate: audio.sample_rate,
                        channels: audio.channels,
                        capture_timestamp: audio.capture_timestamp,
                        device_name: audio.device.to_string(),
                        is_input: matches!(
                            audio.device.device_type,
                            crate::core::device::DeviceType::Input
                        ),
                    };

                    if let Err(e) = qw.write_audio(&packet).await {
                        error!("capture-only: failed to queue audio: {}", e);
                    } else {
                        debug!(
                            "capture-only: queued audio chunk ({} samples, device={})",
                            packet.pcm_samples.len(),
                            packet.device_name
                        );
                        metrics.record_segment_deferred(); // re-use metric as "queued"
                    }
                }
            }));
        }
