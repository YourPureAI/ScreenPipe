...
pub struct AudioManagerOptions {
    ...
    /// When true, raw audio is serialised to `queue_writer` instead of being
    /// passed to Whisper/Deepgram. OCR and transcription are skipped entirely.
    pub capture_only: bool,
    /// Target queue directory writer (only used when `capture_only` is true).
...
impl AudioManagerBuilder {
    ...
    /// Put this `AudioManager` into capture-only mode.
    ///
    /// Raw audio chunks will be serialised via `bincode` and written to the
    /// provided [`screenpipe_queue::QueueWriter`] instead of being transcribed.
    /// Whisper/Deepgram are never loaded in this mode.
    pub fn capture_only(
        mut self,
        queue_writer: std::sync::Arc<screenpipe_queue::QueueWriter>,
    ) -> Self {
        self.options.capture_only = true;
        self.options.queue_writer = Some(queue_writer);
        self
    }
