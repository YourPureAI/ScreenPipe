...
pub struct RecordingConfig {
    ...
    /// Selects the role of this process in the distributed architecture.
    /// `Local` (default) = full pipeline; `CaptureOnly` = client only;
    /// `ProcessOnly` = server only.
    pub operation_mode: OperationMode,

    /// Shared queue directory: written by capture-only, read by process-only.
...
impl RecordingConfig {
    ...
    pub fn from_settings(
        ...
        Self {
            ...
            // Distributed mode defaults — overridden by the CLI layer in
            // RecordArgs::into_recording_config().
    ...
    ///
    /// Pass an optional `queue_writer` to enable capture-only mode
    /// (used when `operation_mode == CaptureOnly`).
    pub fn to_vision_manager_config(
        &self,
        output_path: String,
        vision_metrics: Arc<PipelineMetrics>,
        queue_writer: Option<std::sync::Arc<screenpipe_queue::QueueWriter>>,
    ) -> VisionManagerConfig {
        let capture_only = self.operation_mode == OperationMode::CaptureOnly;
        VisionManagerConfig {
            output_path,
            ignored_windows: self.ignored_windows.clone(),
            included_windows: self.included_windows.clone(),
