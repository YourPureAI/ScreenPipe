...
/// Take a screenshot and walk the accessibility tree **without** running OCR
/// or writing to the database.
///
/// Used by the capture-only monitor loop in [`crate::vision_manager`] to
/// gather the raw material for a [`screenpipe_queue::RawFramePacket`].
///
/// Returns `(image, tree_snapshot, captured_at)`.
pub async fn do_raw_capture(
    monitor: &Arc<SafeMonitor>,
    monitor_id: u32,
    tree_walker_config: &TreeWalkerConfig,
    ...
    image::DynamicImage,
    Option<screenpipe_a11y::tree::TreeSnapshot>,
    chrono::DateTime<chrono::Utc>,
)> {
    use screenpipe_a11y::tree::{TreeWalkResult, TreeWalkerConfig};

    let captured_at = Utc::now();

    // Take the full-screen screenshot
    let (image, _metadata) = capture_monitor_image(monitor)
        .await
        .map_err(|e| anyhow::anyhow!("capture_monitor_image failed on monitor {}: {}", monitor_id, e))?;

    // Walk the accessibility tree on a blocking thread (AX APIs are sync)
    let config = tree_walker_config.clone();
    let tree_walk_result =
        tokio::task::spawn_blocking(move || crate::paired_capture::walk_accessibility_tree(&config))
            .await
            .unwrap_or(TreeWalkResult::NotFound);

    let tree_snap = match tree_walk_result {
        TreeWalkResult::Found(snap) => Some(snap),
        _ => None,
    };

    Ok((image, tree_snap, captured_at))
}
