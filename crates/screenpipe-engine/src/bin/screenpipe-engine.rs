...
use screenpipe_engine::{
    ...
    cli::{
        ...
        Cli, CliAudioTranscriptionEngine, Command, OperationMode,
    },
    hot_frame_cache::HotFrameCache,
    ingest_processor::{start_ingest_processor, IngestProcessorConfig},
...
async fn main() -> anyhow::Result<()> {
    ...
    // ── Distributed mode: create shared QueueWriter ───────────────────────────
    // This is the single queue writer shared by both the audio manager and the
    // vision manager when running in capture-only mode.
    let queue_writer: Option<Arc<screenpipe_queue::QueueWriter>> =
        if config.operation_mode == OperationMode::CaptureOnly
            || config.operation_mode == OperationMode::ProcessOnly
        {
            match &config.queue_dir {
                Some(dir) => match screenpipe_queue::QueueWriter::new(dir) {
                    Ok(qw) => {
                        let qw = Arc::new(qw);
                        info!(
                            "distributed mode: queue dir = {}",
                            dir.display()
                        );
                        Some(qw)
                    }
                    Err(e) => {
                        error!("failed to create queue writer: {}", e);
                        return Ok(());
                    }
                },
                None => {
                    error!(
                        "--mode {:?} requires --queue-dir to be set",
    ...
    // Apply capture-only flag to the audio manager builder
    if config.operation_mode == OperationMode::CaptureOnly {
        if let Some(ref qw) = queue_writer {
            audio_manager_builder = audio_manager_builder.capture_only(qw.clone());
        }
    }
    ...
    let (handle, capture_trigger_tx) = if !config.disable_vision {
        let vision_config = config.to_vision_manager_config(
            ...
            // In capture-only mode, pass the QueueWriter so the lightweight
            // capture-only loop can serialize frames to the queue directory.
            if config.operation_mode == OperationMode::CaptureOnly {
                queue_writer.clone()
            } else {
                None
    ...
    // ── Process-only mode: start the ingest processor ────────────────────────
    // When running as a processing server, the ingest processor is the only
    // active pipeline component on the capture side.  Vision and audio
    // managers are NOT started (the client machines own those).  The ingest
    // processor watches the queue directory for raw packets from thin clients,
    // runs OCR + Whisper, and writes results to the local SQLite DB.
    if config.operation_mode == OperationMode::ProcessOnly {
        if let Some(ref queue_dir) = config.queue_dir {
            let ingest_config = IngestProcessorConfig {
                snapshot_output_path: local_data_dir.join("data"),
            ...
            .await;
            info!(
                "process-only mode: ingest processor started (queue={})",
                queue_dir.display()
            );
        }
    }
